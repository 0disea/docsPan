...
description: "Ejemplo completo de integración con una fintech"
---
...
class PanService {
  ...
  async getOrCreateWallet(userId: string, email?: string): Promise<Wallet> {
    ...
    try {
      ...
      if (error instanceof PanError && error.code === 'WALLET_ALREADY_EXISTS') {
        // Wallet existe pero no está en nuestra DB
  ...
  async deposit(userId: string, amount: number): Promise<Transaction> {
    ...
    try {
      ...
      // Actualizar transacción con intent
...
// Estado de transacción especifica
...
### Registro y Creación de Wallet

```typescript
// El usuario se registra en la app
const user = await registerUser(email, password);

// Automáticamente creamos su wallet pan
const wallet = await panService.getOrCreateWallet(user.id, email);

// Usuario puede ver su dirección para depositos
console.log(`Dirección para depositos: ${wallet.address}`);
```
...
// - La transacción se marca como completada
```
...
Con está implementación, tu fintech puede:

1. **Crear cuentas** automáticamente para cada usuario
2. **Aceptar depositos** en cualquier chain soportada
3. **Generar rendimiento** automático con los mejores APYs
4. **Procesar retiros** de forma instantanea
5. **Mostrar historial** completo de transacciones

Todo esto con una integración simple a pan API, sin necesidad de:
