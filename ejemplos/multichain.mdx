...
description: "Ejemplos de operaciones entre múltiples blockchains"
---

## Caso de Uso

Una plataforma DeFi quiere:
- Permitir depósitos desde cualquier chain
...
<CardGroup cols={2}>
  <Card title="Sin pan" icon="xmark">
    6 pasos manuales: aprobar tokens, configurar bridge, esperar, verificar balance, aprobar para Aave, depositar
  </Card>
  <Card title="Con pan" icon="check">
    1 llamada: `pan.lend()` y listo
  </Card>
</CardGroup>
...
async function depositFromArbitrum(amount: number) {
  ...
  // 3. Esperar finalización del bridge (puede tomar horas)
...
|-------|---------|
| Ethereum: $200 USDC | |
| Arbitrum: $500 USDC | Base: $1000 USDC |
| Base: $300 USDC | 8.5% APY |

pan consolida automáticamente los fondos en la chain con mejor rendimiento.
...
async function getDistributedBalance(walletId: string): Promise<{
  ...
  for (const chainData of balances.chains) {
    const usdc = chainData.tokens.filter(t => t.asset === 'USDC');
    ...
    if (chainAmount > 0) {
      byChain.push({ chain: chainData.chain, amount: chainAmount, inProtocol, available });
      total += chainAmount;
    }
  }

  // Fondos están distribuidos si hay más de 1 chain con balance
...
async function consolidateFunds(
  walletId: string,
  targetChain: string = 'arbitrum' // Chain con gas más barato
): Promise<ConsolidationResult> {
  const balance = await getDistributedBalance(walletId);

  if (!balance.distributed) {
    throw new Error('Fondos ya están consolidados');
  }
...
async function bridgeFunds(params: BridgeParams): Promise<{
  ...
  const sourceChain = balances.chains.find(c => c.chain === params.fromChain);
  const sourceBalance = sourceChain?.tokens.find(t => t.asset === params.asset);
...
function MultiChainDashboard({ walletId }: MultiChainDashboardProps) {
  ...
  async function loadPositions() {
    ...
    for (const chainData of balancesData.chains) {
      for (const token of chainData.tokens as any[]) {
        if (token.asset === 'USDC' && token.protocol) {
          const amount = parseFloat(token.balanceFormatted);
          const apy = token.apy || 0;

          pos.push({
            chain: chainData.chain,
            chainLogo: chainLogos[chainData.chain] || '⬡',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <button
          onClick={() => {/* Abrir modal de optimización */}}
          className="p-4 bg-white rounded-xl shadow hover:shadow-md transition text-left"
        >
...
pan abstrae toda la complejidad de operar en múltiples blockchains, permitiendote enfocarte en la experiencia del usuario.
