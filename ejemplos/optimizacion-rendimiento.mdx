...
- Optimizar automáticamente donde se depositan los fondos
...
class YieldOptimizer {
  ...
  async analyzePositions(walletId: string): Promise<Position[]> {
    ...
    for (const chainData of balances.chains) {
      for (const token of chainData.tokens) {
        if (token.protocol) { // Posición en protocolo
          positions.push({
  ...
  async findRebalanceOpportunities(
    ...
    for (const position of positions) {
      ...
      for (const better of betterYields) {
        ...
        // Días para recuperar costo de gas
  ...
  async executeRebalance(
    ...
    // 2. Depositar en nueva posición (pan automáticamente hace bridge si es necesario)
...
### Dollar Cost Averaging + Optimización
...
async function calculateProjections(
  ...
  // Convertir APY a tasa por periodo de composición
  ...
  for (let m = 1; m <= months; m++) {
    ...
    // APY efectivo considerando composición
...
console.log('Proyección de rendimientos para $10,000:');
console.log('=========================================');
    ...
    <div className="space-y-6">
      ...
      {currentPosition && (
        <div className="bg-white rounded-lg p-6 shadow">
          <h3 className="text-lg font-semibold mb-4">Tu Posición Actual</h3>
      ...
      {currentPosition && bestRate && bestRate.apy > currentPosition.apy && (
        <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg p-6 text-white">
          <h3 className="text-lg font-semibold mb-2">💡 Oportunidad de Optimización</h3>
          <p className="mb-4">
...
Con estas estrategias de optimización puedes:
...
4. **Implementar DCA** con optimización automatica
5. **Proyectar ganancias** para tus usuarios
6. **Mostrar UI interactiva** para optimización manual
