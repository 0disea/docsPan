...
| Código | Causa | Solución |
|--------|-------|----------|
| `INSUFFICIENT_FUNDS` | Sin fondos | Depositar más |
...
| `INSUFFICIENT_CREDITS` | Sin créditos | Comprar |
...
async function ejecutarIntent(params: LendParams) {
  ...
  try {
    ...
    switch (error.code) {
      case 'BRIDGE_FAILED':
        // Los fondos están seguros, se puede reintentar
