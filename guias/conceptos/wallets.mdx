...
```mermaid
flowchart TB
    Wallet["📱 Wallet: pan_wallet_abc123<br/>Address: 0x742d35Cc..."]

    Wallet --> ETH["🔷 Ethereum<br/>200 USDC, 0.5 ETH"]
    Wallet --> ARB["🔵 Arbitrum<br/>500 USDC, 0.1 ETH"]
    Wallet --> BASE["🔵 Base<br/>300 USDC"]
```
...
sequenceDiagram
    ...
    participant Pan as Pan API
    participant Privy as Privy (Custodio)
    participant DB as Base de Datos

    App->>Pan: wallet.create({ userId: '123' })

    rect rgb(255, 243, 224)
        note right of Pan: Creacion de Wallet
        Pan->>Privy: Crear wallet EVM
        Privy->>Privy: Generar llave privada
        Privy->>Privy: Derivar direccion
        Privy->>Privy: Encriptar y almacenar llave
        Privy-->>Pan: Direccion: 0x742d35...
    end

    Pan->>DB: Guardar mapeo userId → walletId
    DB-->>Pan: OK

    Pan-->>App: { id, address, chains[] }

    note over App,Privy: La llave privada NUNCA sale de Privy
```
...
```mermaid
flowchart LR
    subgraph YourApp["Tu Aplicacion"]
        SDK["Pan SDK"]
    end

    subgraph PanAPI["Pan API"]
        WalletService["Wallet Service"]
        IntentService["Intent Service"]
    end

    subgraph Privy["Privy (Custodio)"]
    ...
    SDK -->|API Key| WalletService
    WalletService -->|Crear/Firmar| Privy
    IntentService -->|Ejecutar txn| Privy
    Privy -->|Transacciones firmadas| Chains

    style Privy fill:#e8f5e9,stroke:#388e3c
    style PanAPI fill:#fff3e0,stroke:#f57c00
    style YourApp fill:#e3f2fd,stroke:#1976d2
```
