...
flowchart TB
    ...
    API --> Wallets["Wallets Service"]
    API --> Intents["Intents Service"]
    API --> Yields["Yields Service"]

    Intents --> Planner["Intent Planner"]
    Planner --> Engine["Execution Engine"]

    Engine --> Chains["Blockchains"]
...
Pan usa un modelo de datos jerarquico que conecta desarrolladores, wallets, e intents:

```mermaid
erDiagram
    DEVELOPER ||--o{ API_KEY : tiene
    DEVELOPER ||--o{ WALLET : crea
    WALLET ||--o{ INTENT : ejecuta
    WALLET ||--o{ BALANCE : contiene
...
flowchart TB
    Dev["👤 Developer"]
    Dev --> Keys["🔑 API Keys"]
    Keys --> DevKey["pan_sk_dev_..."]
    Keys --> ProdKey["pan_sk_prod_..."]

    Dev --> W1["Wallet: Usuario A"]
    Dev --> W2["Wallet: Usuario B"]

    W1 --> B1["Balances: 1000 USDC"]
    W1 --> I1["Intents: lend, withdraw"]
```
