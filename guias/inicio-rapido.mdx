...
```javascript JavaScript
const response = await fetch('https://api.pan.tech/v1/demo/fund', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${process.env.PAN_API_KEY}`,
    'Content-Type': 'application/json'
...
const response = await fetch(
  'https://api.pan.tech/v1/balances/pan_wallet_a1b2c3d4e5f6',
  {
    headers: {
      'Authorization': `Bearer ${process.env.PAN_API_KEY}`
    }
...
```javascript JavaScript
const response = await fetch('https://api.pan.tech/v1/intents', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${process.env.PAN_API_KEY}`,
    'Content-Type': 'application/json'
...
<CodeGroup>
```bash cURL
curl -X GET "https://api.pan.tech/v1/intents/intent_xyz789abc123" \
  -H "Authorization: Bearer $PAN_API_KEY"
```
...
async function monitorearIntent(intentId) {
  ...
  while (intentos < maxIntentos) {
    const response = await fetch(
      `https://api.pan.tech/v1/intents/${intentId}`,
      {
        headers: {
          'Authorization': `Bearer ${process.env.PAN_API_KEY}`
        }
...
const BASE_URL = 'https://api.pan.tech/v1';
...
BASE_URL = 'https://api.pan.tech/v1'
...
def main():
    ...
    print('   Fondeo iniciado (1 USDC en arbitrum-sepolia)\n')
